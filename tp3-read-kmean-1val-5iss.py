...
import pandas as pd
path = "./new-data/d32.txt"
databrut = pd.read_csv(path, sep=" ", encoding = "Latin-1", skipinitialspace=True)
#databrut["Géographie"] = databrut["Géographie"].astype("category").cat.codes
#databrut["Ville"] = databrut["Ville"].astype("category").cat.codes

print(databrut.shape)
...
#path = './new-data/'
#databrut = arff.loadarff(open(path+"pluie.csv", 'r'))
...
f0 = datanp_reduced[:,0] # tous les élements de la première colonne
f1 = datanp_reduced[:,1] # tous les éléments de la deuxième colonne
